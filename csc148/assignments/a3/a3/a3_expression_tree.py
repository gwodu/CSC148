...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if isinstance(self._root, int):
            return self._root
        elif isinstance(self._root, str) and\
                self._root not in [OP_ADD, OP_MULTIPLY]:
            return lookup[self._root]
        else:
            subtree_value_list = []
            for subtree in self._subtrees:
                subtree_value_list.append(subtree.eval(lookup))
            if self._root == OP_ADD:
                return sum(subtree_value_list)
            else:
                return _multiply_list(subtree_value_list)
    ...
    def __str__(self) -> str:
        ...
        elif isinstance(self._root, str) and self._root\
                not in [OP_ADD, OP_MULTIPLY]:
            return self._root
        else:
            str_subtree_list = []
            for subtree in self._subtrees:
                str_subtree_list.append(str(subtree))
            string = "(" + (" " + self._root + " ").join(str_subtree_list) + ")"
            return string
    ...
    def substitute(self, from_to: Dict[Union[str, int], Union[str, int]])\
            -> None:
        """
    ...
    def _height(self) -> int:
        ...
        4
        """
        if self._root not in [OP_ADD, OP_MULTIPLY]:
            return 1
        else:
            counter_list = []
            for subtree in self._subtrees:
                counter = 1
                counter += subtree._height()
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        True
        """
        if isinstance(self._root, str) and self._root\
                not in [OP_ADD, OP_MULTIPLY]\
                and self._root not in lookup:
            lookup[self._root] = 0
        for subtree in self._subtrees:
...
def _multiply_list(list_: List[int]) -> int:
    """Return the product of all variables in the list"""
    result = 1
    for num in list_:
        result *= num
    return result
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    (3 + (a * b) + a + (5 + c))
    """
    daddy_t = ExprTree(values[0][0], [])
    queue = Queue()
    queue.enqueue(daddy_t)
    i = 1
    while i < len(values):
        curr_tree = queue.dequeue()
        for value in values[i]:
            new_expr_t = ExprTree(value, [])
            curr_tree.append(new_expr_t)
            if value in [OP_ADD, OP_MULTIPLY]:
                queue.enqueue(new_expr_t)
        i += 1
    return daddy_t
