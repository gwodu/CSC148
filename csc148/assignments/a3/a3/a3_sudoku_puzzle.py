...
class SudokuPuzzle(Puzzle):
    ...
    def extensions(self) -> List[SudokuPuzzle]:
        ...
        Precondition: Assume the puzzle is correct so far.
    ...
    def fail_fast(self) -> bool:
        ...
        You can assume that this puzzle state is valid.
        ...
        for r in range(len(grid)):
            for c in range(len(grid[r])):
                if self._fail_fast_helper(grid, r, c):
                    return True
        return False

    def _fail_fast_helper(self, grid: List[List[str]], r: int, c: int) -> bool:
        """Returns True if <allowed_symbols> is empty"""
    ...
    def has_unique_solution(self) -> bool:
        ...
        while len(solver.solve(self, seen_solution_set.copy())) != 0:
            solution = solver.solve(self, seen_solution_set.copy())[-1]
            i += 1
            seen_solution_set.add(str(solution))
...
if __name__ == "__main__":
    ...
    python_ta.check_all(config={
        ...
        'allowed-import-modules': [
            'doctest', 'python_ta', 'typing', 'puzzle', '__future__',
            'a3_solver'
        ],
        'disable': ['E1136'],
        'max-attributes': 15
