...
class Tweet:
    ...
    def __str__(self):
        ...
        >>> print(t1)
        Rukhsana, (2017-09-16), Hey!
        """
        return str(self.userid + ',' + ' ' + '(' +self.created_at.isoformat() + \
               ')' + ',' + ' ' + self.content)
...
class User:
    ...
    def retweet(self, tweet: Tweet, new_date: date) -> Tweet:
        ...
        >>> u1 = User('Diane', 'amazing laugh')
        >>> u1.retweet(t1, date(2021, 5, 13))
        'Diane', datetime.date(2021, 5, 13), 'David is so cool!'
        """
        self.tweet(tweet.content)
        return self.userid + new_date.isoformat() + tweet.content
