...
class RecursiveList:
    ...
    def insert(self, index: int, item: Any) -> None:
        ...
        if index == 0:
            ...
            self._first = item
            self._rest = rest
        elif self._first is None and index > 0:
            raise IndexError
        else:
    ...
    def map(self, f: Callable[[Any], Any]) -> RecursiveList:
        ...
        if self._first is None:
            ...
            new_node = RecursiveList([])
            new_node._first = f(self._first)
            new_node._rest = self._rest.map(f)
            return new_node
    ...
    def selections(self) -> List[RecursiveList]:
        ...
        if self.is_empty():
            ...
            new_seletions = []
            for pos_sel in rest_selections:
                new_rest = _copy_rec_list(pos_sel)
                new_list = RecursiveList([])
                new_list._rest = new_rest
                new_list._first = self._first
                new_seletions.append(new_list)

            return new_seletions + rest_selections

def _copy_rec_list(input_list: RecursiveList) -> RecursiveList:
    if input_list.is_empty():
        return RecursiveList([])
    else:
        new_list = RecursiveList([])
        new_list._first = input_list._first
        new_list._rest = _copy_rec_list(input_list._rest)
        return new_list
