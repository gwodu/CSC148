...
class BinarySearchTree:
    ...
    def height(self) -> int:
        ...
        if self.is_empty():
            ...
            lcount = 1
            rcount = 1
            lcount += self._left.height()
            rcount += self._right.height()
    ...
    def rotate_right(self) -> None:
        ...
        """
        pivot = self._left
        moving_tree = self._left._right
        ex_root_tree = BinarySearchTree(self._root)
        ex_root_tree._right = self._right

        self._root, self._left, self._right = pivot._root, pivot._left, ex_root_tree
        self._right._left._root, self._right._left._left, self._right._left._right \
            = moving_tree._root, moving_tree._left, moving_tree._right
    ...
    def rotate_left(self) -> None:
        ...
        """
        ex_root_tree = BinarySearchTree(self._root)
        ex_root_tree._left = self._left
        ex_root_tree._right = self._right._left # adds moving tree to ex_tree
        new_root_tree = BinarySearchTree(self._right._root)
        new_root_tree._right = self._right._right


        self._root, self._right, self._left \
            = new_root_tree._root, new_root_tree._right, ex_root_tree
